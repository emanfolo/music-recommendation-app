    ...
    <div className=" flex flex-col justify-center items-center min-h-screen min-w-full bg-gray-200">
      ...
      <div className=" flex justify-center flex-wrap max-w-md items-center ">
        {spotifySeedGenres.map((genre, index) => (
          ...
          >
            <div className={`w-3 h-3 opacity-0 ${selectedGenres.includes(genre) && "opacity-100"}`}><Tick/></div>
            {genre}
          </div>
        ))}
      </div>
      <div>
      <button className="m-8 p-4 border  rounded-full bg-white text-black" onClick={() => router.push('/')} >Restart</button>
      <button className="m-8 p-4 border  rounded-full bg-blue-500 text-black" onClick={handleConfirm} >Create Playlist</button>
      </div>
    </div>
  );
};
